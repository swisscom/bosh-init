...
BOSH_CLI_DIR=/home/ubuntu/go/src/github.com/cloudfoundry/bosh-init

#inside the docker container
BOSH_PRIVATE_KEY_DIR=/home/ubuntu/private_keys
PRIVATE_KEY_DIR=$(cd $(dirname $BOSH_LITE_PRIVATE_KEY) && pwd)
BOSH_LITE_PRIVATE_KEY_BASENAME=$(basename $BOSH_LITE_PRIVATE_KEY)
BOSH_LITE_PRIVATE_KEY=${BOSH_PRIVATE_KEY_DIR}/${BOSH_LITE_PRIVATE_KEY_BASENAME}
...
docker run \
  ...
  -e BOSH_STEMCELL_URL \
  -e BOSH_CPI_RELEASE_URL \
  -v $SRC_DIR:$BOSH_CLI_DIR \
  -v $PRIVATE_KEY_DIR:$BOSH_PRIVATE_KEY_DIR \
  $DOCKER_IMAGE \
  $BOSH_CLI_DIR/bin/test-acceptance-with-vm --provider=aws \
  &
